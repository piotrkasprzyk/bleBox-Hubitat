...
def off() {
	logDebug("off")
	parent.childCommand(getDataValue("channel"), "00000000")
}

def setWhite(level) {
	//	Allows independent setting of white level.
	logDebug("setWhite: ${level})")
	level = (level * 2.55).toInteger()
	level = hubitat.helper.HexUtils.integerToHexString(level, 1)
	parent.childCommand(getDataValue("channel"), state.savedLevel[0..5] + level)
}
...
def setColor(color) {
	...
	if (saturation < 1) { saturation = hue }
	...
	//	Initial setting of white level to minimum of R, G, B.
	rgbw += hubitat.helper.HexUtils.integerToHexString(rgbData.min(), 1)
	parent.childCommand(getDataValue("channel"), rgbw, state.fadeSpeed)
	state.FadeSpeed = 2
}
